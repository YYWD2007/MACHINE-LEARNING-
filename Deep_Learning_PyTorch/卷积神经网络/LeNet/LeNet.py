...
class LeNet:
    def __init__(self, kernel_size, kernel_size_pool, padding, stride,
        ...
        nn.Conv2d(1, 6, kernel_size=self.kernel_size, padding=self.padding), nn.Sigmoid(), # 输入通道数字，输出，padding(填充)
        nn.AvgPool2d(kernel_size=self.kernel_size_pool, stride=self.stride), #stride=步幅，AvgPool2d → 取平均值，MaxPool2d → 取最大值
        nn.Conv2d(6, 16, kernel_size=self.kernel_size), nn.Sigmoid(),
        nn.AvgPool2d(kernel_size=self.kernel_size_pool, stride=self.stride),
        nn.Flatten(),
        nn.Linear(self.n_hiddens1, self.n_hiddens2), nn.Sigmoid(),
        nn.Linear(self.n_hiddens2, self.n_hiddens3), nn.Sigmoid(),
